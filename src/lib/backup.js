...
export function exportBackup(data) {
    downloadText(`parkledger-backup-${new Date().toISOString().slice(0, 10)}.json`, createBackup(data), "application/json");
}
export function exportLegacyTxt(data) {
    const lines = [
        "PARKLEDGER BACKUP",
        `VERSION=${BACKUP_VERSION}`,
    ...
    downloadText(`parkledger-legacy-${new Date().toISOString().slice(0, 10)}.txt`, lines.join("\n"));
}

...
export function AdminLogin() {
  ...
  const go = async () => {
    ...
    try {
      ...
      const authorized = !!signedInUser.email && signedInUser.emailVerified &&
        signedInUser.email.trim().toLowerCase() === ADMIN_EMAIL.trim().toLowerCase();

      if (!authorized) {
        // Never reveal which account is expected — a generic message only.
        await signOut(auth);
        setMessage("This account is not authorised.");
        return;
      }

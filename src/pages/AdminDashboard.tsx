...
import { Plus, Save, Trash2, UserPlus, RefreshCw, Download, Upload, ShieldCheck, Receipt, Users, Wallet, Pencil } from "lucide-react";
...
export function AdminDashboard({ section = "overview" }: { section?: string }) {
  ...
  return <>{toast && <div className="toast"><ShieldCheck size={17} />{toast}</div>}
    ...
    {section === "flats" && <SimpleAdmin title="Flats" items={data.flats} fields={["number", "label", "active"]} collection="flats" notify={notify} onEdit={(item: any) => setEditItem({ item, collection: "flats" })} />}
    {section === "charges" && <ChargesAdmin data={data} notify={notify} />}
    {section === "categories" && <SimpleAdmin title="Categories" items={data.categories} fields={["name", "color", "active"]} collection="categories" notify={notify} onEdit={(item: any) => setEditItem({ item, collection: "categories" })} />}
    ...
    <Modal open={!!editItem} title="Edit Record" onClose={() => setEditItem(null)}>
      {editItem && <GenericEditForm item={editItem.item} collection={editItem.collection} onDone={() => setEditItem(null)} notify={notify} />}
    </Modal>
  </>;
}
...
function SimpleAdmin({ title, items, fields, collection, notify, onEdit }: any) { const [name, setName] = useState(""); return <section className="panel"><div className="panel-head"><div><h2>{title}</h2><p>Manage {title.toLowerCase()} without hardcoding</p></div></div><div className="inline-form"><input value={name} onChange={e => setName(e.target.value)} placeholder={`${title.slice(0, -1)} name`} /><button className="primary-btn" onClick={async () => { if (!name.trim()) return; await saveDoc(collection, undefined, { [fields[0]]: sanitize(name), active: true }); setName(""); notify(`${title.slice(0, -1)} added.`) }}><Plus size={15} /> Add</button></div><div className="simple-list">{items.map((x: any) => <div key={x.id} className="simple-row"><div><strong>{x[fields[0]]}</strong><small>{x.label || x.color || ""}</small></div><div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}><span className="section-tag">{x.active ? "Active" : "Inactive"}</span><button className="icon-btn" onClick={() => onEdit(x)}><Pencil size={15} /></button></div></div>)}</div></section> }
